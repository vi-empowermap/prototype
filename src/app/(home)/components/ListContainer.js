...
const ListContainer = ({turnOnMap, doubleScreenTouched, getData, bundeslandtext }) => {
  const ready = useRecoilValue(readyAniAtom)
  return (
    <div id="listContainer" className={`relative w-full bg-white lg:border-l-2 border-black lg:max-w-[calc(200px+20vw)] ${!doubleScreenTouched ? "h-1/2 min-h-[50vh]" : "h-[100px]"} lg:h-screen z-[1300] ${turnOnMap ? "block" : "hidden"}  ${ready ? "opacity-100": "opacity-0"}`}>
      <div id="listContainer2"  className={`absolute top-0 lg:-left-8 w-full lg:max-w-[calc(200px+20vw)] ${!doubleScreenTouched ? "h-full" : "h-[100px]"} lg:h-screen pt-20 relative overflow-y-scroll z-[1300] no-scrollbar `}>
        ...
        {getData.map((value, index) => {
          return <ListBox key={index} index={index} value={value} bundeslandtext={bundeslandtext} />;
        })}
      </div>
    </div>
  );
