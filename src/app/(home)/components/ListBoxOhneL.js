...
const ListBoxOhneL = ({ index, value }) => {
  ...
  return (
    <div className="w-full aspect-square relative group">
      ...
      <div
        style={{ borderColor: `${value.bgColor}`, backfaceVisibility: "hidden" }}
        onClick={() => onClick(value)}
        className={`absolute top-0 left-0 flex flex-col py-2 px-2 gap-4 w-full aspect-square border-2 border-black cursor-pointer rounded-2xl group-hover:[transform:rotateY(180deg)] transition-all bg-white duration-700`}
      >
        <div className="flex flex-wrap gap-2 justify-center items-center">
